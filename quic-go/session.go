...
import (
	...
	"github.com/stormlin/qperf/quic-go/quictrace"
)
...
type session struct {
	...
	// 该 channel 只有 1 个单位容量，作为不同 go 程之间的信号量使用
...
func (s *session) handleSinglePacket(p *receivedPacket, hdr *wire.Header) bool /* was the packet successfully processed */ {
	...
	if err != nil {
		...
		case handshake.ErrOpenerNotYetAvailable:
			// Sealer for this encryption level not yet available. Try again later.
			wasQueued = true
...
// 发送一个数据包，返回值包含是否发送数据包，以及发送过程是否出错
