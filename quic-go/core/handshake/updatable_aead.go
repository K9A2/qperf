...
import (
	...
	"github.com/stormlin/qperf/quic-go/core/congestion"
	"github.com/stormlin/qperf/quic-go/core/qerr"
...
func (a *updatableAEAD) Open(dst, src []byte, rcvTime time.Time, pn protocol.PacketNumber, kp protocol.KeyPhaseBit, ad []byte) ([]byte, error) {
	//fmt.Println("in updatable_aead open")
...
func (a *updatableAEAD) Seal(dst, src []byte, pn protocol.PacketNumber, ad []byte) []byte {
	//fmt.Println("in updatable_aead.go Seal")
	if a.firstSentWithCurrentKey == protocol.InvalidPacketNumber {
	...
	// The AEAD we're using here will be the qtls.aeadAESGCM13.
	// It uses the nonce provided here and XOR it with the IV.
	result := a.sendAEAD.Seal(dst, a.nonceBuf, src, ad)
	//fmt.Printf("src len: %d, dst len: %d, result len: %d, ad len: %d\n", len(src), len(dst), len(result), len(ad))
	return result
	//return src // 不加密
}
