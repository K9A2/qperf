...
func dialContext(
	ctx context.Context, 		// 由多个 goroutine 共享的程序上下文
	pconn net.PacketConn,   // 通用连接实例
	remoteAddr net.Addr, 		// 对端地址, 包含协议, 地址和端口号
	host string, 						// 地址与端口号
	tlsConf *tls.Config,   	// TLS 设置
	config *Config,					// 设置 QUIC 连接所需要的上下文
	createdPacketConn bool,
) (Session, error) {
	if tlsConf == nil {
		// 未提供 TLS 信息
	...
	// 在 QUIC 的多路复用器中增加一条连接, 并返回一个 packetHandlers 实例 (packetHandlerManager)
...
func (c *client) dial(ctx context.Context) error {
	c.logger.Infof("Starting new connection to %s (%s -> %s), " +
		"source connection ID %s, destination connection ID %s, version %s",
